...
for skill in "${required_skills[@]}"; do
  ...
  if grep -q -E '^name:[[:space:]]*[^[:space:]].*$' "$file"; then
    pass "$skill has name field"
  else
    fail "$skill missing name field"
  fi

  if grep -q -E '^description:[[:space:]]*[^[:space:]].*$' "$file"; then
...
for pattern in "${legacy_patterns[@]}"; do
  if grep -q -r -F "$pattern" "$AGENT_DIR/skills"; then
...
for pattern in "${mapping_checks[@]}"; do
  if grep -q -E "$pattern" "$AGENT_DIR/AGENTS.md"; then
