...
(function() {
    ...
    function getImportRouteCost(resName) {
        try {
            ...
            console.warn("[Evolve Auto-Buy] Error calling game tradeBuyPrice:", e);
        }
        if (!window.evolve || !window.evolve.global || !window.evolve.global.resource || !window.evolve.global.resource[resName]) return 0;
        let uPrice = getUnitPrice(resName) || 0;
        let ratio = window.evolve.tradeRatio ? (window.evolve.tradeRatio[resName] || 0) : 0;
        return uPrice * ratio;
    }
    ...
    function manageTradeRoutes(isSaving = false) {
        ...
        // Calculate the cash income constraint to prevent negative net cash flow.
        let currentCashIncome = liveGlobal.resource.Money.diff || 0;
        
        // Calculate the cost of currently active import routes
        ...
        // If we are saving, preserve a larger portion of base cash income (e.g. 25% or at least 1.0/s)
        // If not saving, keep at least 0.1/s to prevent bankruptcy.
        let minCashIncome = isSaving ? Math.max(1.0, baseCashIncome * 0.25) : 0.1;
        // -----------------------------------
            ...
            if (group.length > 1 && idx < 999 && (settings.purchaseStrategy === 'time_saved' || settings.purchaseStrategy === 'balanced')) {
                ...
                while (groupRemaining > 0) {
                    ...
                    if (baseCashIncome - (projectedImportCost + routeCost) < minCashIncome) {
                        break; // Stop allocating routes if we cannot afford them
                    }
                ...
                for (let cand of group) {
                    ...
                    if (routeCost > 0) {
                        ...
                        if (maxAffordable <= 0) {
                            continue; // Cannot afford even one route of this resource
                        }
    ...
    function runAutoBuy() {
        ...
        if (moneyTarget) {
            if (moneyAmount < moneyTargetCost) {
                ...
                // Still run queue helpers and trade route management, but skip purchases!
                try {
                    autoCraftForQueue();
                } catch (e) {
                ...
                <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 0.5rem; flex-wrap: wrap; gap: 0.5rem;">
                    <div style="display: inline-flex; align-items: center; gap: 6px;">
                        ...
                        <h3 class="title is-5" style="margin: 0; display: inline-flex; align-items: center; gap: 6px; cursor: pointer; user-select: none;" id="autobuy-title-clickable">
                            Auto-Buy Automation <span style="font-size: 0.75rem; color: #888; font-weight: normal;">v1.15.4</span>
                        </h3>
                    </div>
