...
(function() {
    ...
    function manageTradeRoutes(isSaving = false) {
        ...
        const liveGlobal = getRealGlobal() || global;
        const topBarVm = document.querySelector('#topBar') ? document.querySelector('#topBar').__vue__ : null;
        if (!topBarVm || topBarVm.race['no_trade'] || global.settings.pause) return;
    ...
    function runAutoBuy() {
        if (window.isEvolveMadCompanionActive || document.getElementById('mad-companion-panel')) {
            updateStatus("CONFLICT");
            console.warn("[Evolve Auto-Buy] MAD Companion detected! Auto-Buy core loop is paused to prevent conflicts.");
            return;
        }
        ...
        if (!nextItem && global.r_queue && global.r_queue.queue && global.r_queue.queue.length > 0) {
    ...
    function injectUI() {
        ...
        const isConflict = window.isEvolveMadCompanionActive || document.getElementById('mad-companion-panel');
        ...
        if (!document.getElementById('autobuy-dashboard')) {
            ...
            dashboard.innerHTML = `
                ${isConflict ? '<div class="challenge-alert" style="margin-bottom: 0.5rem; animation: flash-border 1.5s infinite alternate; border: 1px solid #ff3860; padding: 8px; border-radius: 4px; background-color: rgba(255, 56, 96, 0.1); color: #ff3860; font-weight: bold; text-align: center;">⚠️ CONFLICT DETECTED!<br>MAD Farm Companion is active! Auto-Buy is paused.</div>' : ''}
                <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 0.5rem; flex-wrap: wrap; gap: 0.5rem;">
                    <div style="display: inline-flex; align-items: center; gap: 6px;">
                        ...
                        Auto-Buy Automation <span style="font-size: 0.75rem; color: #888; font-weight: normal;">v1.20.0</span>
                        </h3>
                    </div>
                    <div style="display: inline-flex; align-items: center; gap: 12px;">
                        <label class="checkbox" style="font-weight: 500; cursor: pointer; user-select: none; margin: 0; display: inline-flex; align-items: center; gap: 6px; ${isConflict ? 'opacity: 0.5; pointer-events: none;' : ''}">
                            <input id="autobuy-master-toggle" type="checkbox" style="width: 16px; height: 16px; cursor: pointer;" ${isConflict ? 'disabled' : ''} />
                            <span>Enabled</span>
                        </label>
                        <span id="autobuy-status" class="tag is-dark" style="font-weight: bold; margin: 0;">Status: ${isConflict ? 'CONFLICT' : 'Idle'}</span>
                    </div>
                </div>
