...
(function () {
        ...
        try {
          if (window.LZString) {
            ...
            if (state && state.settings && !state.settings.expose) {
              ...
              let compressed = window.LZString.compressToUTF16(
                JSON.stringify(state),
              );
              localStorage.setItem("evolved", compressed);
              val = compressed;
              console.log(
                "[Evolve Interceptor] Intercepted save and forced settings.expose = true",
              );
            }
          }
        } catch (e) {
  ...
  function getGameStateCacheKey() {
    ...
    if (global.queue && global.queue.queue) {
      parts.push(
        "q:" + global.queue.queue.map((i) => i.id + ":" + (i.q || 1)).join(","),
      );
    }
    ...
    if (global.prestige) {
      if (global.prestige.Plasmid)
        parts.push("p:" + global.prestige.Plasmid.count);
      if (global.prestige.Phage)
        parts.push("ph:" + global.prestige.Phage.count);
      if (global.prestige.CRISPR)
        parts.push("c:" + JSON.stringify(global.prestige.CRISPR));
    }
  ...
  function getBasePrice(res, global) {
    let r_val =
      resourceBaselines[res] ||
      (global.resource && global.resource[res]
        ? global.resource[res].value
        : 0);
    ...
    if (res === "Titanium") {
      if (
        ...
        global.tech["titanium"] > 0
      ) {
        r_val *=
          global.resource &&
          global.resource.Alloy &&
          global.resource.Alloy.display
            ? 1
            : 2.5;
      } else {
        r_val *= 5;
  ...
  function getFathomCheck(race, global) {
    ...
    if (
      ...
      const torturerWorkers =
        global.civic.torturer && global.civic.torturer.workers
          ? global.civic.torturer.workers
          : 0;
  ...
  function getQueueItemCosts(item) {
    ...
    if (
      item.action === "arpa" ||
      (segments[0] && segments[0].substring(0, 4) === "arpa")
    ) {
      ...
      if (project && project.cost) {
        let complete = global.arpa[projectType]
          ? global.arpa[projectType].complete
          : 0;
    ...
    // 3. Hell Mechs
      ...
      item.action === "hell-mech" ||
      (segments[0] === "hell" &&
        segments[1] &&
        segments[1].substring(0, 4) === "mech")
    ) {
      let size = item.type && item.type.size ? item.type.size : item.size || 0;
      let infernal =
        item.type && item.type.infernal !== undefined
          ? item.type.infernal
          : item.infernal || false;
      ...
      if (actions[cat]) {
        if (cat === "city" || cat === "evolution" || cat === "starDock") {
          ...
          Object.keys(actions[cat]).forEach(function (region) {
            if (
              actions[cat][region] &&
              actions[cat][region].hasOwnProperty(structKey)
            ) {
  ...
  function isResourceBuyable(resName, global) {
    ...
    const galacticBuyResources = new Set([
      "Deuterium",
      "Neutronium",
      "Adamantite",
      "Elerium",
  ...
  function getQueueNeeds() {
    ...
    const buildQueue =
      global.queue && global.queue.queue && !global.queue.pause
        ? global.queue.queue
        : [];
    const researchQueue =
      global.r_queue && global.r_queue.queue && !global.r_queue.pause
    ...
    logDebug(
      `getQueueNeeds() starting queue scan of length ${scanLength}. Virtual starting amounts: ${JSON.stringify(virtualAmounts)}`,
    );
    ...
    for (let i = 0; i < scanLength; i++) {
      ...
      logDebug(
        `  Item [${i}]: ${item.label || item.id} (${entry.type}, originalIndex: ${entry.originalIndex}). Costs: ${JSON.stringify(costs)}`,
      );
      ...
      for (let res of Object.keys(costs)) {
        ...
        if (
          ...
          costVal > global.resource[res].max
        ) {
          hasHardBlock = true;
          blockedResources.add(global.resource[res].name || res);
          logDebug(
            `    Hard block: cost of ${res} (${costVal}) exceeds storage cap (${global.resource[res].max})`,
          );
        }
      }
      ...
      function recordNeed(resName, amt) {
        if (amt > 0) {
          ...
          logDebug(
            `    Recorded need: ${resName} += ${amt} (First needed at item [${tempFirstNeededIndex[resName]}], primary: ${tempPrimaryNeeds[resName]})`,
          );
        }
      }
      ...
      for (let j = 0; j < resKeys.length; j++) {
        ...
        if (craftCosts.hasOwnProperty(res) && !global.race["no_craft"]) {
          ...
          logDebug(
            `    Resolving craftable ${res}: cost ${costVal}, virtual available ${tempVirtualAmounts[res] || 0}, missingAmt ${missingAmt}`,
          );
          if (missingAmt > 0) {
            ...
            if (firstValidIndex === -1 && entry.originalIndex < entry.horizon) {
              ...
              recipe.forEach((ingredient) => {
                currentReservedPrecursors[ingredient.r] =
                  (currentReservedPrecursors[ingredient.r] || 0) +
                  craftActionsNeeded * ingredient.a;
              });
            }
            ...
            if (maxAllowedActions > 0) {
              ...
              recipe.forEach((ingredient) => {
                tempVirtualAmounts[ingredient.r] =
                  (tempVirtualAmounts[ingredient.r] || 0) -
                  maxAllowedActions * ingredient.a;
              });
              tempVirtualAmounts[res] =
                (tempVirtualAmounts[res] || 0) + maxAllowedActions * mult;
              logDebug(
                `      Executed ${maxAllowedActions} virtual crafts of ${res}. Virtual precursor counts: ${JSON.stringify(tempVirtualAmounts)}`,
              );
            }
            ...
            if (remainingActions > 0) {
              ...
              for (let k = 0; k < recipe.length; k++) {
                ...
                if (
                  global.resource[ingredient.r] &&
                  global.resource[ingredient.r].max > 0 &&
                  ingredient.a > global.resource[ingredient.r].max
                ) {
                  itemFailed = true;
                  blockedResources.add(
                    global.resource[ingredient.r].name || ingredient.r,
                  );
                  logDebug(
                    `      Hard block: single craft precursor ${ingredient.r} cost (${ingredient.a}) exceeds cap (${global.resource[ingredient.r].max})`,
                  );
                }
                let precursorCost = remainingActions * ingredient.a;
                let missingP =
                  precursorCost -
                  Math.max(0, tempVirtualAmounts[ingredient.r] || 0);
                if (missingP > 0) {
                  const isBuyable = isResourceBuyable(ingredient.r, global);
                  logDebug(
                    `      Precursor ${ingredient.r} missing: ${missingP} (buyable: ${isBuyable})`,
                  );
                  if (isBuyable) {
                ...
                tempVirtualAmounts[ingredient.r] =
                  (tempVirtualAmounts[ingredient.r] || 0) - precursorCost;
              }
              tempVirtualAmounts[res] =
                (tempVirtualAmounts[res] || 0) + remainingActions * mult;
            }
          }
          tempVirtualAmounts[res] = Math.max(
            0,
            (tempVirtualAmounts[res] || 0) - costVal,
          );
        } else {
          let missingAmt = costVal - Math.max(0, tempVirtualAmounts[res] || 0);
          logDebug(
            `    Resolving non-craftable ${res}: cost ${costVal}, virtual available ${tempVirtualAmounts[res] || 0}, missingAmt ${missingAmt}`,
          );
          if (missingAmt > 0) {
            const isBuyable = isResourceBuyable(res, global);
            logDebug(
              `      Non-craftable ${res} missing: ${missingAmt} (buyable: ${isBuyable})`,
            );
            if (isBuyable) {
              recordNeed(res, missingAmt);
            }
          ...
          if (firstValidIndex === -1 && entry.originalIndex < entry.horizon) {
            currentReservedPrecursors[res] =
              (currentReservedPrecursors[res] || 0) + costVal;
          }
        }
      ...
      logDebug(
        `    Item [${i}] resolved successfully. Committed virtual amounts: ${JSON.stringify(virtualAmounts)}`,
      );
    }
    ...
    logDebug(
      `getQueueNeeds() finished. Combined Needs: ${JSON.stringify(queueNeeds)}, Allowed Crafts: ${JSON.stringify(allowedCrafts)}, Blocked: ${JSON.stringify(window.autobuy_blocked_resources)}, firstValidIndex: ${currentFirstValidIndex}`,
    );
    return {
      needs: queueNeeds,
      firstNeededIndex: firstNeededIndex,
  ...
  function autoCraftForQueue(moneyTarget = null) {
    ...
    if (!settings.enabled || global.settings.pause || global.race["no_craft"])
      return;

    const craftCosts = window.evolve.craftCost || {};

    logDebug(
      `autoCraftForQueue running. moneyTarget: ${moneyTarget ? moneyTarget.label || moneyTarget.id : "None"}. allowedCrafts: ${JSON.stringify(currentAllowedCrafts)}`,
    );
    ...
    currentAllowedCrafts.forEach((craft) => {
      ...
      logDebug(
        `  Evaluating craft for ${resName}. Priority: ${craft.priority}. Initial allowed actions: ${allowedActions}`,
      );
      ...
      function protectItem(item) {
        ...
        try {
          ...
          Object.keys(costs).forEach((res) => {
            protectedResources[res] =
              (protectedResources[res] || 0) + costs[res];
          });
        } catch (e) {
          console.error(
            "[Evolve Auto-Buy] Error evaluating protected resources in autoCraftForQueue:",
            e,
          );
        }
      }
      ...
      if (recipe && Array.isArray(recipe)) {
        for (let ingredient of recipe) {
          ...
          if (protectedResources.hasOwnProperty(reqRes)) {
            ...
            const currentAmt = global.resource[reqRes]
              ? global.resource[reqRes].amount
              : 0;
            ...
            if (allowedActions < originalAllowed) {
              console.log(
                `[Evolve Auto-Buy] Crafting of ${resName} limited by protected precursor ${reqRes}: protected ${requiredAmt}, current ${currentAmt}. Reduced allowed actions from ${originalAllowed} to ${allowedActions}`,
              );
            }
            logDebug(
              `    Protecting ingredient ${reqRes}: recipe req ${ingredient.a}, protected ${requiredAmt}, current ${currentAmt}, safeToSpend ${safeToSpend}. Reduced allowedActions from ${originalAllowed} to ${allowedActions}`,
            );
          }
        }
      }
      ...
      if (allowedActions <= 0) {
        logDebug(
          `    No allowed actions remaining for ${resName} after protection checks.`,
        );
        return;
      }
      ...
      if (!resEl || !resEl.__vue__) {
        logDebug(
          `    Cannot craft ${resName}: sidebar DOM element or Vue instance not found.`,
        );
        return;
      }
      ...
      if (recipe && Array.isArray(recipe)) {
        recipe.forEach((ingredient) => {
          precursorAmountsBefore[ingredient.r] = global.resource[ingredient.r]
            ? global.resource[ingredient.r].amount
            : 0;
        });
      }
      ...
      try {
        ...
        logDebug(
          `    Executing craft for ${resName}: actions = ${allowedActions}`,
        );
        ...
        if (produced > 0) {
          console.log(
            `[Evolve Auto-Buy] Auto-crafted ${produced.toFixed(0)} ${resName} to fulfill queue needs.`,
          );
          ...
          if (Math.abs(actualMult - expectedMult) > 0.01) {
            console.log(
              `[Evolve Auto-Buy] Crafting yield mismatch for ${resName}! Expected multiplier: ${expectedMult.toFixed(3)}, Actual: ${actualMult.toFixed(3)}. Updating multiplier.`,
            );
          ...
          if (recipe && Array.isArray(recipe)) {
            for (let ingredient of recipe) {
              ...
              let after = global.resource[ingredient.r]
                ? global.resource[ingredient.r].amount
                : 0;
              ...
              if (Math.abs(consumed - expectedConsumed) > 0.01) {
                console.log(
                  `[Evolve Auto-Buy] Precursor ${ingredient.r} consumption mismatch for ${resName}! Expected: ${expectedConsumed}, Actual: ${consumed}.`,
                );
                mismatchDetected = true;
                break;
              }
          ...
          let isCapped =
            global.resource[resName] &&
            global.resource[resName].max > 0 &&
            amountAfter >= 0.999 * global.resource[resName].max;
    ...
    Object.keys(PRECURSOR_TO_CRAFTABLE).forEach((precursor) => {
      ...
      if (maxActions > 0) {
        ...
        try {
          global.settings.mKeys = false;
          logDebug(
            `    Executing excess precursor craft for ${craftRes}: actions = ${maxActions}`,
          );
          resEl.__vue__.craft(craftRes, maxActions);
          let produced = maxActions * getCraftMultiplier(craftRes);
          console.log(
            `[Evolve Auto-Buy] Auto-crafted ${produced.toFixed(0)} ${craftRes} from excess precursor ${precursor}.`,
          );
        } catch (e) {
          console.error(
            `[Evolve Auto-Buy] Error crafting excess precursor ${craftRes}:`,
            e,
          );
        } finally {
          global.settings.mKeys = origMKeysVal;
    ...
    if (
      ...
      if (actions[cat].hasOwnProperty(type)) {
        ...
        Object.keys(actions[cat]).forEach((region) => {
          if (
            actions[cat][region] &&
            typeof actions[cat][region] === "object" &&
            actions[cat][region].hasOwnProperty(type)
          ) {
  ...
  function getBuildingCosts(c_action) {
    ...
    try {
      if (
        c_action.id.startsWith("arpa-") ||
        (c_action.id.startsWith("arpa") && !c_action.id.startsWith("arpa-"))
      ) {
        let projectKey = c_action.id.startsWith("arpa-")
          ? c_action.id.substring(5)
          : c_action.id.substring(4);
        let complete =
          global.arpa && global.arpa[projectKey]
            ? global.arpa[projectKey].complete
            : 0;
        let inc = Math.min(1, 100 - complete);
        if (inc <= 0) return {};
  ...
  function isBuildingBuyable(c_action) {
    ...
    if (
      result &&
      c_action.hasOwnProperty("condition") &&
      !c_action.condition()
    ) {
      result = false;
    }
  ...
  function purchaseResource(resName) {
    const topBarVm = document.querySelector("#topBar")
      ? document.querySelector("#topBar").__vue__
      : null;
    ...
    logDebug(
      `purchaseResource(${resName}) starting. value: ${value}, current amount: ${resVm.amount}, current money: ${moneyVm.amount}`,
    );
    ...
    if (neededForQueue > 0) {
      ...
      logDebug(
        `  Resource is enqueued need. neededForQueue: ${neededForQueue}, targetQty set to: ${targetQty}`,
      );
    } else {
      ...
      let storageChunk = 0;
      if (resVm.max > 0) {
        storageChunk = Math.ceil(resVm.max * 0.1);
      }

      let cashFraction = isCapSafeMode ? 0.5 : 0.2;
      let affordableChunk = Math.floor((moneyVm.amount * cashFraction) / value);

      let calculatedChunk =
        storageChunk > 0
          ? Math.min(storageChunk, affordableChunk)
          : affordableChunk;
      calculatedChunk = Math.max(calculatedChunk, userQty, 100);

      targetQty = Math.max(targetQty, calculatedChunk);
      logDebug(
        `  Resource is not queue need. storageChunk: ${storageChunk}, affordableChunk: ${affordableChunk}, targetQty set to: ${targetQty}`,
      );
    }

    let availableMoney = moneyVm.amount;
    if (currentMoneyTargetCost > 0) {
      const isTargetPrecursor =
        currentQueueNeeds &&
        currentQueueNeeds[resName] > 0 &&
        currentFirstNeededIndex &&
        currentFirstNeededIndex[resName] === currentFirstValidIndex;
      if (!isTargetPrecursor) {
        availableMoney -= currentMoneyTargetCost;
        logDebug(
          `  Protecting moneyTargetCost: subtracted ${currentMoneyTargetCost} from availableMoney because ${resName} is not a precursor of nextItem.`,
        );
      } else {
        logDebug(
          `  Allowing spend of protected money on target precursor ${resName}.`,
        );
      }
    }
    logDebug(
      `  availableMoney: ${availableMoney} (after protecting currentMoneyTargetCost: ${currentMoneyTargetCost})`,
    );
    if (availableMoney <= 0) {
    ...
    let amount = Math.floor(
      Math.min(targetQty, availableMoney / value, resVm.max - resVm.amount),
    );
    logDebug(
      `  Attempting to buy amount: ${amount} (min of targetQty ${targetQty}, money-cap ${Math.floor(availableMoney / value)}, storage-cap ${resVm.max - resVm.amount})`,
    );
    if (amount > 0) {
      resVm.amount += amount;
      ...
      const randVal = Math.rand
        ? Math.rand(1000, 10000)
      ...
      console.log(
        `[Evolve Auto-Buy] Purchased ${amount} ${resName} at $${value.toFixed(2)}/unit`,
      );
      return true;
    }
  ...
  function getImportVolumePerRoute(resName, global) {
    ...
    if (
      global.race["persuasive"] &&
      window.evolve.traits &&
      window.evolve.traits.persuasive
    ) {
      rate *=
        1 +
        (window.evolve.traits.persuasive.vars()[0] *
          global.race["persuasive"]) /
          100;
    }
    if (
      global.race["merchant"] &&
      window.evolve.traits &&
      window.evolve.traits.merchant
    ) {
    ...
    if (
      ...
      window.evolve.traits &&
      window.evolve.traits.ocular_power
    ) {
  ...
  function getPriorityScore(cand, strategy, global) {
    ...
    let maxStorage = global.resource[resName]
      ? global.resource[resName].max
      : 1;
  ...
  function getLiveGalaxyTrade() {
    const el =
      document.getElementById("galaxyTrade") ||
  ...
  function resetImportRoutes() {
    ...
    if (changed) {
      ...
      console.log(
        "[Evolve Auto-Buy] Reset auto-managed import trade routes to 0.",
      );
    }
  }
  ...
  function getExportVolumePerRoute(resName, global) {
    ...
    if (
      global.stats &&
      global.stats.achieve &&
      global.stats.achieve.hasOwnProperty("trade")
    ) {
  ...
  function resetAutoExportRoutes() {
    ...
    try {
      localStorage.setItem(
        "evolve_autobuy_auto_exports",
        JSON.stringify(autoExportRoutes),
      );
    } catch (e) {
    ...
    if (changed) {
      ...
      console.log(
        "[Evolve Auto-Buy] Reset auto-managed export trade routes to 0.",
      );
    }
  }
  ...
  function getPiracyMultiplier(region, global) {
    ...
    switch (region) {
      case "gxy_stargate":
        pirate =
          0.1 * (instinct ? global.tech.piracy * 0.9 : global.tech.piracy);
        pillage = 0.5;
        break;
      case "gxy_gateway":
        pirate =
    ...
    let num_def_plat_on =
      global.galaxy && global.galaxy.defense_platform
    ...
    let num_starbase_on =
      global.galaxy && global.galaxy.starbase ? global.galaxy.starbase.on : 0;
    if (region === "gxy_gateway" && num_starbase_on) {
      armada += num_starbase_on * 25;
    }

    let num_foothold_on =
      global.galaxy && global.galaxy.foothold ? global.galaxy.foothold.on : 0;
    if (region === "gxy_alien2" && num_foothold_on) {
      armada += num_foothold_on * 50;
      let num_armed_miner_on = global.galaxy.armed_miner
    ...
    if (region === "gxy_chthonian") {
      let num_minelayer_on = global.galaxy.minelayer
  ...
  function getGalacticSellVol(idx, global) {
    ...
    if (
      vueEl &&
      vueEl.__vue__ &&
      vueEl.__vue__.$options &&
      vueEl.__vue__.$options.filters &&
      vueEl.__vue__.$options.filters.s_vol
    ) {
      try {
  ...
  function getGalacticBuyVol(idx, global) {
    ...
    if (
      ...
      vueEl.__vue__.$options &&
      vueEl.__vue__.$options.filters &&
      vueEl.__vue__.$options.filters.t_vol
    ) {
      try {
  ...
  function resetGalacticRoutes() {
    if (
      ...
      !window.evolve.global.galaxy ||
      !window.evolve.global.galaxy.trade
    )
      return;
    const global = window.evolve.global;
    const liveGlobal = getRealGlobal() || global;
    ...
    if (changed) {
      ...
      console.log(
        "[Evolve Auto-Buy] Reset auto-managed galactic trade routes to 0.",
      );
    }
  }
  ...
  function manageGalacticTradeRoutes(isSaving = false) {
    ...
    const minPiracy =
      settings.minGalacticTradePiracy !== undefined
        ? settings.minGalacticTradePiracy
        : 0.5;
    ...
    for (let i = 0; i < offers.length; i++) {
      ...
      let currentPrice =
        getUnitPrice(buyRes) || getBasePrice(buyRes, liveGlobal);
      ...
      const cand = {
        ...
        ratio: ratio,
        firstNeededIndex:
          currentFirstNeededIndex[buyRes] !== undefined
            ? currentFirstNeededIndex[buyRes]
            : 999,
      };
      ...
      for (let cand of list) {
        ...
        if (targetIndex === 999) {
          ...
          if (queueNeedsHistory.length > 0) {
            reservedAmt =
              queueNeedsHistory[queueNeedsHistory.length - 1][sellRes] || 0;
          }
        ...
        if (currentQueueNeeds && currentQueueNeeds[sellRes] > 0) {
          let sellFirstNeeded =
            currentFirstNeededIndex[sellRes] !== undefined
              ? currentFirstNeededIndex[sellRes]
              : 999;
        ...
        let curAmt = liveGlobal.resource[sellRes]
          ? liveGlobal.resource[sellRes].amount
          : 0;
        ...
        if (liveGlobal.resource[sellRes]) {
          ...
          if (sellNeededEarlier) {
            ...
            let baseDiff =
              (liveGlobal.resource[sellRes].diff || 0) +
              currentRoutes * sellVol;
    ...
    if (changed) {
      ...
      let routeSummary = Object.keys(targetRoutes)
        ...
        .join(", ");
      console.log(
        `[Evolve Auto-Buy] Auto-managed galactic trade routes: [ ${routeSummary || "None"} ]`,
      );
    }
  }
  ...
  function manageTradeRoutes(isSaving = false) {
    ...
    let routeCap =
      liveGlobal.tech.currency >= 6
        ? 1000000
        : liveGlobal.tech.currency >= 4
          ? 100
          : 25;
    ...
    if (isSaving) {
      ...
      if (currentMoneyTarget) {
        try {
          ...
          console.error(
            "[Evolve Auto-Buy] Error getting protected costs in trade routes:",
            e,
          );
        }
      }
      ...
      Object.keys(liveGlobal.resource).forEach((resName) => {
        ...
        let maxExportableRoutes = Math.max(
          0,
          Math.floor(naturalDiff / exportRate),
        );

        // Cap by tech cap and manual export space
        let manualExp = manualExportRoutes[resName] || 0;
        maxExportableRoutes = Math.min(
          maxExportableRoutes,
          routeCap - manualExp,
        );
        ...
        const sellPrice =
          window.evolve && typeof window.evolve.tradeSellPrice === "function"
            ? window.evolve.tradeSellPrice(resName)
            : 0;
      ...
      Object.keys(liveGlobal.resource).forEach((resName) => {
        ...
        const isRouteAllowed =
          (hasTradeTech && !liveGlobal.race["terrifying"]) ||
          (hasBananaTrait && resName === "Food");
        if (isBuyable && isRouteAllowed) {
          ...
          const cand = {
            ...
            ratio: ratio,
            firstNeededIndex:
              currentFirstNeededIndex[resName] !== undefined
                ? currentFirstNeededIndex[resName]
                : 999,
          };
      ...
      const sortQueueCandidates = (list) => {
        list.sort((a, b) => {
          ...
          let scoreA = getPriorityScore(
            a,
            settings.purchaseStrategy,
            liveGlobal,
          );
          let scoreB = getPriorityScore(
            b,
            settings.purchaseStrategy,
            liveGlobal,
          );
      ...
      if (
        liveGlobal.race &&
        liveGlobal.race.governor &&
        liveGlobal.race.governor.g &&
        liveGlobal.race.governor.g.bg === "entrepreneur"
      ) {
        let totalImportAllowed = settings.autoSellCapped
          ? totalManualExportRoutes
          : totalExportRoutes;
      ...
      allCandidates.forEach((cand) => {
        let idx =
          cand.firstNeededIndex !== undefined ? cand.firstNeededIndex : 999;
        if (!groups[idx]) {
          groups[idx] = [];
        }
      ...
      let sortedGroupIndices = Object.keys(groups)
        .map(Number)
        .sort((a, b) => a - b);
      ...
      for (let idx of sortedGroupIndices) {
        ...
        if (
          group.length > 1 &&
          idx < 999 &&
          (settings.purchaseStrategy === "time_saved" ||
            settings.purchaseStrategy === "balanced")
        ) {
          let groupRoutes = {};
          group.forEach((cand) => {
          ...
          while (groupRemaining > 0) {
            ...
            for (let cand of group) {
              ...
              let amount = liveGlobal.resource[resName]
                ? liveGlobal.resource[resName].amount
                : 0;
              let maxStorage = liveGlobal.resource[resName]
                ? liveGlobal.resource[resName].max
                : 1;
              let totalNeed = currentQueueNeeds[resName] || 0;
              let effectiveNeed = Math.max(1, Math.min(totalNeed, maxStorage));

              let missing = Math.max(0, effectiveNeed - amount);
              let genRate = liveGlobal.resource[resName]
                ? liveGlobal.resource[resName].diff
                : 0;

              let currentRoutes =
                liveGlobal.resource[resName] &&
                liveGlobal.resource[resName].trade > 0
                  ? liveGlobal.resource[resName].trade
                  : 0;
              let baseGenRate =
                genRate -
                currentRoutes * getImportVolumePerRoute(resName, liveGlobal);
              let simulatedGenRate =
                baseGenRate +
                groupRoutes[resName] *
                  getImportVolumePerRoute(resName, liveGlobal);
            ...
            if (
              baseCashIncome - (projectedImportCost + routeCost) <
              minCashIncome
            ) {
              break;
            }
          ...
          for (let cand of group) {
            ...
            if (routeCost > 0) {
              let maxAffordable = Math.floor(
                (baseCashIncome - minCashIncome - projectedImportCost) /
                  routeCost,
              );
              if (maxAffordable <= 0) {
                continue;
      ...
      if (settings.autoSellCapped && remainingRoutes > 0) {
        ...
        Object.keys(liveGlobal.resource).forEach((resName) => {
          ...
          let maxExportableRoutes = Math.max(
            0,
            Math.floor(baseDiff / exportRate),
          );
          let manualExp = manualExportRoutes[resName] || 0;
          maxExportableRoutes = Math.min(
            maxExportableRoutes,
    ...
    if (changed) {
      ...
      let routeSummary = Object.keys(targetImportRoutes)
        ...
        .concat(
          Object.keys(manualExportRoutes).map(
            (k) => `${k} (manual): -${manualExportRoutes[k]}`,
          ),
        )
        .concat(
          Object.keys(targetExportRoutes).map(
            (k) => `${k} (auto-sell): -${targetExportRoutes[k]}`,
          ),
        )
        .join(", ");
      console.log(
        `[Evolve Auto-Buy] Auto-managed trade routes: [ ${routeSummary || "None"} ]`,
      );
    }
  }
  ...
  function updateStatus(text) {
    ...
    if (statusEl) {
      ...
      if (
        text.startsWith("Buying") ||
        text.startsWith("Cap-Safe") ||
  ...
  function runAutoBuy() {
    ...
    if (
      ...
      try {
        ...
        if (!bestBuilding) {
          ...
          const knowledgeBuildings =
            getStorageIncreasingBuildingsForResource("Knowledge");
        ...
        if (bestBuilding) {
          ...
          if (bestBuilding.powered) {
            powerCost =
              typeof bestBuilding.powered === "function"
                ? bestBuilding.powered()
                : bestBuilding.powered;
          }

          const currentPower =
            global.city && typeof global.city.power !== "undefined"
              ? global.city.power
              : 0;
          if (powerCost > 0 && powerCost > currentPower) {
            ...
            if (validGenerators.length > 0) {
              ...
              console.log(
                `[Evolve Auto-Buy] Target building ${bestBuilding.id} requires ${powerCost}MW, but available power is only ${currentPower}MW. Switching to power generator: ${chosenGen.id}`,
              );
              bestBuilding = chosenGen;
            } else {
              console.log(
                `[Evolve Auto-Buy] Target building ${bestBuilding.id} requires ${powerCost}MW, but available power is only ${currentPower}MW and no power generator is buyable.`,
              );
              bestBuilding = null;
            }
          }
        ...
        if (bestBuilding) {
          ...
          const type = segments[1];
          const label =
            typeof bestBuilding.title === "string"
              ? bestBuilding.title
              : bestBuilding.title();
          ...
          const liveQueue =
            queueEl && queueEl.__vue__ ? queueEl.__vue__.$data.queue : null;
          if (liveQueue) {
            liveQueue.push({
              id: queueId,
              action: action,
            ...
            console.log(
              `[Evolve Auto-Buy] Automatically enqueued storage expansion: ${label} (${queueId})`,
            );
            ...
            console.log(
              `[Evolve Auto-Buy] Automatically enqueued storage expansion (fallback): ${label} (${queueId})`,
            );
            updateStatus(`Queued Storage: ${label}`);
          }
          return;
    ...
    if (
      ...
      global.r_queue &&
      global.r_queue.queue &&
      global.r_queue.queue.length > 0 &&
      !global.r_queue.pause
    ) {
    ...
    if (nextItem) {
      logDebug(
        `runAutoBuy() selected nextItem: ${nextItem.label || nextItem.id} (action: ${nextItem.action}, type: ${nextItem.type})`,
      );
      try {
        ...
        if (onlyNeedsMoneyOrKnowledge) {
          if (moneyCost > 0 && moneyCost <= moneyMax) {
            ...
            logDebug(
              `  moneyTarget set: ${moneyTargetName}, cost: ${moneyTargetCost}, onlyNeedsMoneyOrKnowledge: true`,
            );
          }
          if (knowledgeCost > 0) {
            firstItemNeedsKnowledge = true;
            logDebug(
              `  firstItemNeedsKnowledge set: true (needs ${knowledgeCost} Knowledge)`,
            );
          }
        }
      } catch (e) {
    ...
    if (firstItemNeedsKnowledge) {
      ...
      if (rGlobal && rGlobal.arpa && rGlobal.arpa.sequence) {
        ...
        if (changed) {
          console.log(
            "[Evolve Auto-Buy] Paused sequencing & auto-sequence to accumulate Knowledge for queued item.",
          );
      ...
      if (localStorage.getItem("autobuy_paused_sequencing") === "true") {
        ...
        if (
          rGlobal &&
          rGlobal.arpa &&
          rGlobal.arpa.sequence &&
          !rGlobal.arpa.sequence.on
        ) {
    ...
    if (moneyTarget) {
      ...
      if (moneyAmount < moneyTargetCost) {
        ...
        updateStatus(
          `Saving: ${moneyTargetName} ($${moneyAmount.toFixed(0)}/$${moneyTargetCost.toFixed(0)})`,
        );
        logDebug(
          `Entering Saving mode for target ${moneyTargetName}. Money target cost: ${moneyTargetCost}, current money: ${moneyAmount}`,
        );
        ...
        try {
          ...
          console.error(
            "[Evolve Auto-Buy] Error managing galactic trade routes:",
            e,
          );
        }
        ...
        logDebug(
          `Entering Building mode for target ${moneyTargetName}. Money: ${moneyAmount} / ${moneyTargetCost}. Only needs money/knowledge.`,
        );
        return;
      }
    } else {
    ...
    Object.keys(global.resource).forEach((resName) => {
      ...
      if (!resVm.display || (resVm.max > 0 && resVm.amount >= 0.99 * resVm.max))
        return;
    ...
    if (isCapSafeMode) {
      ...
      if (targetedCandidates.length > 0) {
        ...
        for (let cand of targetedCandidates) {
          if (
            isResourceCooledDown(cand.name, cand.currentPrice, cand.basePrice)
          ) {
            let success = purchaseResource(cand.name);
            if (success) {
              if (moneyTarget && moneyAmount < moneyTargetCost) {
                updateStatus(
                  `Saving: ${moneyTargetName} (Buying: ${cand.name})`,
                );
              } else {
      ...
      for (let cand of targetedCandidates) {
        ...
        let isCooledDown = isResourceCooledDown(
          cand.name,
          cand.currentPrice,
          cand.basePrice,
        );
        ...
        let priceAllowed =
          cand.currentPrice <= safetyCeiling || moneyAmount >= 0.95 * moneyMax;
      ...
      for (let cand of queueCandidates) {
        ...
        const isTargetPrecursor =
          currentQueueNeeds &&
          currentQueueNeeds[cand.name] > 0 &&
          currentFirstNeededIndex &&
          currentFirstNeededIndex[cand.name] === currentFirstValidIndex;
        let isDecaying = isPriceDecaying(cand.name);
        if (isTargetPrecursor) {
          isDecaying = false;
        }
      ...
      if (moneyTarget && moneyAmount < moneyTargetCost) {
        ...
        updateStatus(
          race["no_trade"] ? "Monitoring (No Free Trade)" : "Monitoring",
        );
      }
    }
  }
  ...
  function injectUI() {
    ...
    // Optimization: Skip injection work if dashboard is already fully built and checks injected
    {
      ...
      const marketItems = market.querySelectorAll(
        '.market-item[id^="market-"]',
      );
      if (
        dashboard &&
        checkboxes.length === marketItems.length &&
    ...
    if (!document.getElementById("autobuy-dashboard")) {
      ...
      if (debugToggle) {
        ...
        debugToggle.addEventListener("change", (e) => {
          ...
          saveSettings();
          console.log(
            `[Evolve Auto-Buy] Debug logging: ${settings.debugLogging}`,
          );
        });
      }

      const minPiracySelect = document.getElementById(
        "autobuy-min-piracy-select",
      );
      if (minPiracySelect) {
        minPiracySelect.value =
          settings.minGalacticTradePiracy !== undefined
            ? settings.minGalacticTradePiracy
            : 0.5;
        minPiracySelect.addEventListener("change", (e) => {
          ...
          saveSettings();
          console.log(
            `[Evolve Auto-Buy] Changed min galactic trade piracy to: ${settings.minGalacticTradePiracy}`,
          );
        });
      }
      ...
      if (settingsTitle)
        settingsTitle.addEventListener("click", toggleSettings);
      ...
      const detailsTitle = document.getElementById(
        "autobuy-details-title-clickable",
      );
    ...
    marketItems.forEach((item) => {
      ...
      cb.style.cssText =
        "cursor: pointer; margin: 0; width: 13px; height: 13px;";
      ...
      priceLabel.style.cssText =
        "font-size: 0.725rem; margin-left: 6px; font-family: monospace; padding-left: 6px;";
  ...
  function updateItemLabels() {
    ...
    items.forEach((item) => {
      ...
      if (currentPrice !== null) {
        ...
        if (
          settings.resources[resName] ||
          (currentQueueNeeds && currentQueueNeeds[resName] > 0)
        ) {
          if (!isCooledDown) {
            statusText = " [Spiked]";
  ...
  function updateQueueListUI() {
    ...
    const global = window.evolve.global;
    const bQueueEmpty =
      !global.queue || !global.queue.queue || global.queue.queue.length === 0;
    const rQueueEmpty =
      !global.r_queue ||
      !global.r_queue.queue ||
      global.r_queue.queue.length === 0;
    ...
    if (keys.length === 0) {
      ...
      listStr = keys
        .map(
          (k) =>
            `${global.resource[k] ? global.resource[k].name : k}: ${needs[k].toFixed(0)}`,
        )
        .join(", ");
    ...
    if (
      window.autobuy_blocked_resources &&
      window.autobuy_blocked_resources.length > 0
    ) {
  ...
  function updatePiracyDisplay() {
    ...
    const localMult =
      stargateMult > 0 ? overallMult / stargateMult : overallMult;
